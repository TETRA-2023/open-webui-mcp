...
Re-snapshot of `patches/specs/open-webui.openapi.json` from the deployed OpenWebUI's `/openapi.json` after `0.2.2-1` shipped. The earlier snapshot lagged live OWUI by a few minutes and missed: 1 removed op (`GET /api/v1/retrieval/`), 4 operationId renames on `/openai/{path}` per-method, and body-schema drift on `/api/v1/evaluations/feedback` (POST + `{id}/POST`) and `/api/v1/users/default/permissions` (GET + POST). All four classes detected by the new drift tooling shipped in `dbd40d7`. Drift report after re-snapshot: clean, 459 shared ops with no body-schema deltas.

### Updated (TETRA-side)
- `patches/specs/open-webui.openapi.json` — re-captured from the deployed OpenWebUI's `/openapi.json` on 2026-05-10. Net diff vs `-1`: -1 op, 4 operationId renames, 4 schema bodies updated.

## [0.2.2-1] — 2026-05-10

TETRA-side OpenAPI spec patch on top of unchanged upstream pin `85d88af`. Upstream's bundled snapshot (committed 2026-04-07) lags the OpenWebUI version we deploy: the deployed OpenWebUI's `/openapi.json` reports 460 operations vs. 422 bundled, with body-schema additions to `/api/v1/auths/admin/config` (new required `ENABLE_AUTOMATIONS`, `ENABLE_CALENDAR`) that block configuration writes through the wrapper. Default policy in CONTRIBUTING.md is to wait for upstream re-snapshot, but upstream has not moved since the initial pin, so this release applies the documented fallback (b): snapshot the spec ourselves.

### Added (TETRA-side)
- `patches/specs/open-webui.openapi.json` — OpenAPI snapshot captured from the deployed OpenWebUI's `/openapi.json` on 2026-05-10. 957 KB, 2-space indent, sort_keys to match upstream formatting.
